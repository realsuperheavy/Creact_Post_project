...
import { PROMOTE_TYPE_LABELS } from '../../constants/promoteTypes';
import { http } from '../../services/HttpService';
import { copyTextToClipboard } from '../../utils/common';
export default {
  components: { GeneratorTools },
  name: 'ExplorerCopywritings',
  data() {
    return {
      query: '',
      results: [],
      loading: false,
      filteredData: [],
      messageTypes: PROMOTE_TYPE_LABELS,
    };
  },
  mounted() {
    this.loadData();
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        this.results = promotes.map((item) => ({
          ...item,
          truncated: true,
        }));
        this.filterData();
