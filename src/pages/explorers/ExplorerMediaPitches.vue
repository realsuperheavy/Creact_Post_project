  ...
  <div class="explorer-container">
    ...
    <div v-if="!loading">
      <div class="item-info" v-for="(item, idx) in filteredData" :key="idx">
        <div class="item-title" v-if="item.baseData.journalistName">
          {{ item.baseData.journalistName }}
        </div>
        ...
        <div class="d-flex flex-wrap">
          <div class="item-property mr-3">
            ...
            {{ item.baseData.company }}
          </div>
          <div class="item-property mr-3">
            <span class="label">Your Name: </span>
            {{ item.baseData.name }}
          </div>
          <div class="item-property" v-if="item.baseData.publication">
            <span class="label">Publication: </span>
            {{ item.baseData.publication }}
          </div>
        </div>
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        ...
        this.results = mediaPitches.map((item) => ({
          ...item,
          truncated: true,
        }));
        this.filterData();
