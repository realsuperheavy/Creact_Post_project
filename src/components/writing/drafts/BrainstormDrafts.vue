<template>
  <div class="bg-light rounded">
    ...
    <VueSlickCarousel
      ...
      v-if="!loading && answers.length > 0"
    >
      <div class="article-item" v-for="(item, idx) in answers" :key="idx">
        <b-card>
          <draft-tools
            :hideView="true"
    ...
    async loadData() {
      ...
      try {
        ...
        this.answers = answers.map((item) => ({
          ...item,
          sm: this.$func.formttedSubstring(item.answer, strLimit),
        }));
