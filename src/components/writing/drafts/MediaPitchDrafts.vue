<template>
  <div class="bg-light rounded">
    ...
    <div
      ...
      v-if="!loading && mediaPitches.length === 0"
    >
      There are no saved Media Pitches.
    </div>
    <VueSlickCarousel
      :arrows="false"
      :dots="true"
      ...
      v-if="!loading && mediaPitches.length > 0"
    >
      <div class="article-item" v-for="(item, idx) in mediaPitches" :key="idx">
        <b-card>
          <draft-tools
            :hideView="true"
...
export default {
  ...
  methods: {
    ...mapActions({
      setMediaPitch: 'draftStore/setMediaPitch',
    }),
    async loadData() {
      const strLimit = 200;
      try {
        ...
        const {
          data: { mediaPitches },
        } = await http().get('/media-pitches', {
          params: { query: this.query },
        });
        this.mediaPitches = mediaPitches.map((item) => ({
          ...item,
    ...
    goToDetail(item) {
      this.setMediaPitch(item);
      this.$router.push({ name: routerItems.MEDIA_PITCH });
    },
    async deleteItem(item) {
      try {
        await http().delete('media-pitch/' + item._id);
        this.loadData();
        // eslint-disable-next-line no-empty
      } catch (err) {}
    },
