<template>
  <div class="bg-light rounded">
    ...
    <div
      class="w-100 d-flex py-4 my45 justify-content-center"
      v-if="!loading && questions.length === 0"
    >
      There are no saved questions.
    </div>
    <VueSlickCarousel
      :arrows="false"
      :dots="true"
      ...
      v-if="!loading && questions.length > 0"
    >
      <div class="article-item" v-for="(item, idx) in questions" :key="idx">
        <b-card>
          <draft-tools
            :hideView="true"
...
export default {
  ...
  methods: {
    ...
    async deleteItem(item) {
      try {
        await http().delete('question/' + item._id);
        this.loadData();
        // eslint-disable-next-line no-empty
      } catch (err) {}
    },
