<template>
  <b-form>
    <div class="row quote-form">
      ...
      <div class="col-11">
        <b-form-group label="Select a Tool"><span><span class="toolavail">Available tools: 16</span>
          <b-form-select
            v-model="form.type"
            class="form-control"
...
export default {
  ...
  data() {
    return {
      form: {
        type: SUMMARY_TYPES.SIMPLIFY,
        description: '',
      },
     SUMMARY_TYPES: SUMMARY_TYPE_LABELS,
     summaryTypes: Object.keys(SUMMARY_TYPE_LABELS).map((key) => ({
  ...
  methods: {
    async onSubmit() {
      ...
      try {
        const {
          data: { results },
        } = await http().post('/doc/generate-summary', this.form);
